...
* </ol>
* Both methods return a query language sting and a list of values for filling named parameters. For example the
* following query and parameter list might be returned:
* 
* <pre>
...
* </pre>
* 
* This is an abstract class. A subclass must be used to implement individual query languages. Currently only HQL query
* language is supported ( <code>com.googlecode.genericdao.dao.hibernate.HibernateSearchToQLProcessor</code>). The that
* implementation could be used for EQL query language as well with no or minor modifications.
*/
...
public abstract class BaseSearchProcessor {
   ...
   /**
    * This is the string used to represent the root entity of the search within the query. The default value is
    * <code>"_it"</code>. It may be necessary to use a different alias if there are entities in the data model with the
   ...
   /**
    * Generate the QL string for a given search. Fill paramList with the values to be used for the query. All
    * parameters within the query string are specified as named parameters ":pX", where X is the index of the parameter
    * value in paramList.
    */
   ...
   /**
    * Generate the QL string that will query the total number of results from a given search (paging is ignored). Fill
    * paramList with the values to be used for the query. All parameters within the query string are specified as named
    * parameters ":pX", where X is the index of the parameter value in paramList.
    * 
   ...
   /**
    * Internal method for generating from clause. This method should be called after generating other clauses because
    * it relies on the aliases they create. This method takes every path that is called for in the other clauses and
    * makes it available as an alias using left joins. It also adds join fetching for properties specified by
    * <code>fetches</code> if <code>doEagerFetching</code> is <code>true</code>. <b>NOTE:</b> When using eager
    * fetching, <code>applyFetches()</code> must be executed first.
    */
   ...
   /**
    * Internal method for generating the join portion of the from clause. This method should be called after generating
    * other clauses because it relies on the aliases they create. This method takes every path that is called for in
    * the other clauses and makes it available as an alias using left joins. It also adds join fetching for properties
    * specified by <code>fetches</code> if <code>doEagerFetching</code> is <code>true</code> . <b>NOTE:</b> When using
    * eager fetching, <code>applyFetches()</code> must be executed first.
    */
   ...
   /**
    * Generate a QL string for a subquery on the given property that uses the given filter. This is used by SOME, ALL
    * and NONE filters.
    * 
    * @param ctx
    *            - a new context just for this sub-query
    * @param property
    *            - the property of the main query that points to the collection on which to query
    * @param filter
   ...
   /**
    * <p>
    * In the case of simple ALL/SOME/NONE filters, a simpler hql syntax is used (which is also compatible with
    * collections of values). Simple filters include ALL/SOME/NONE filters with exactly one sub-filter where that
    * filter applies to the elements of the collection directly (as opposed to their properties) and the operator is =,
    * !=, <, <=, >, or >=.
    * 
    * <p>
    * For example:
    ...
    * </pre>
    * 
    * If the filter meets these criteria as a simple ALL/SOME/NONE filter, the QL string for the filter will be
    ...
    * @param operation
    *            - a string used to fill in the collection operation. The value should be either "some" or "all".
    */
    ...
    * 
    * @param isCollection
    *            <code>true</code> if the value is a collection of values, for example with IN and NOT_IN operators.
   ...
   /**
    * Used by {@link #negate(Filter)}. There's a complication with null values in the database so that !(x == 1) is not
    * the opposite of (x == 1). Rather !(x == 1 and x != null) is the same as (x == 1). This method applies the null
    * check explicitly to all filters included in the given filter tree.
    */
   ...
   /**
    * append a custom expression to the string builder, replacing any property tokens (i.e "{prop}") with a reference
    * to the property.
    */
   ...
   /**
    * append a custom expression to the string builder, replacing any property tokens (i.e "{prop}") with a reference
    * to the property and value tokens (i.e. "?n") with params.
    */
   protected void appendCustomExpression(StringBuilder sb, SearchContext ctx, String expression, List<?> values) {
	...
	while (matcher.find()) {
		...
		if (values == null || values.isEmpty()) {
		    throw new IllegalArgumentException("This custom filter expression (" + expression + ") calls for a value placeholder number " + valueIndex
			    + ", but no values were specified.");
		}
		if (valueIndex > values.size()) {
		    throw new IllegalArgumentException("This custom filter expression (" + expression + ") calls for a value placeholder number " + valueIndex
			    + ", but only " + values.size() + " values were specified.");
		}
   ...
   /**
    * Given a full path to a property (ex. department.manager.salary), return the reference to that property that uses
    * the appropriate alias (ex. a4_manager.salary).
    */
   ...
   /**
    * Split a path into two parts. The first part will need to be aliased. The second part will be a property of that
    * alias. For example: (department.manager.salary) would return [department.manager, salary].
    */
   protected String[] splitPath(SearchContext ctx, String path) {
	...
	if (pos == -1) {
		...
		if (pos != -1 && lastSegment.equals("size") && metadataUtil.get(ctx.rootClass, currentPath.substring(0, pos)).isCollection()) {
		    first = true;
		}
   ...
   /**
    * Given a full path to an entity (ex. department.manager), return the alias to reference that entity (ex.
    * a4_manager). If there is no alias for the given path, one will be created.
    */
	...
	return SearchUtil.walkFilters(filters, new SearchUtil.FilterVisitor() {
		...
		if (filter.getValue() != null) {
			...
			if (filter.getValue() instanceof List) {
			    ...
			    throw new IllegalArgumentException("The search has a filter (" + filter
				    + ") for which the value should be a List of Filters but is not a list. The actual type is " + filter.getValue().getClass());
			}
